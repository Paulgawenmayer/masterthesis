"""
This script downloads the latest DOP in grayscale format for a given coordinate
or bounding box in Baden-Wuerttemberg.
"""
import os
import sys
...
def get_LGL_grayscale_DOP_by_coord(latitude, longitude, output_dir=None):
    """
    Downloads grayscale DOP for a single coordinate point.
...
def get_LGL_grayscale_DOP_by_bbox(lat1, lon1, lat2, lon2, output_dir=None):
    """
    Downloads grayscale DOP for a bounding box defined by two coordinates:
    ...
    try:
        ...
        filename = f"{lat1:.6f}_{lon1:.6f}__{lat2:.6f}_{lon2:.6f}_LGL_Grayscale.jpg"
        filepath = os.path.join(output_dir, filename)
...
def get_input():
    ...
    try:
        ...
        if len(parts) == 2:
            ...
            lat, lon = parts
            print(f"\nDownloading grayscale DOP for coordinate: {lat}, {lon}")
            return get_LGL_grayscale_DOP_by_coord(lat, lon)
            
        elif len(parts) == 4:
            # Bounding box mode
            lat1, lon1, lat2, lon2 = parts
            print(f"\nDownloading grayscale DOP for bounding box: ({lat1}, {lon1}) to ({lat2}, {lon2})")
            return get_LGL_grayscale_DOP_by_bbox(lat1, lon1, lat2, lon2)
