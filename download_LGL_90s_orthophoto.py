"""
This script downloads DOP images from the 1990s for a given coordinate or bounding box
...
def get_LGL_1990s_DOP_by_coord(latitude, longitude, output_dir=None):
    """
    Downloads DOP images from the 1990s for a single coordinate point.
    ...
    try:
        ...
        if not found_usable_image:
            print("\n❌ No usable image found for given coordinates in any year of the 90s")
            return False
        return True
...
def get_LGL_1990s_DOP_by_bbox(lat1, lon1, lat2, lon2, output_dir=None):
    """
    Downloads DOP images from the 1990s for a bounding box defined by two coordinates:
    ...
    try:
        ...
        if not found_usable_image:
            print("\n❌ No usable image found for given bbox in any year of the 90s")
            return False
        return True
...
def get_input():
    ...
    try:
        ...
        if len(parts) == 2:
            ...
            lat, lon = parts
            print(f"\nDownloading 1990s DOP for coordinate: {lat}, {lon}")
            return get_LGL_1990s_DOP_by_coord(lat, lon)
            
        elif len(parts) == 4:
            # Bounding box mode
            lat1, lon1, lat2, lon2 = parts
            print(f"\nDownloading 1990s DOP for bounding box: ({lat1}, {lon1}) to ({lat2}, {lon2})")
            return get_LGL_1990s_DOP_by_bbox(lat1, lon1, lat2, lon2)
