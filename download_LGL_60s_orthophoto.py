"""
This script downloads DOP images from the 1960s for a given coordinate or bounding box
...
def get_LGL_1960s_DOP_by_coord(latitude, longitude, output_dir=None):
    """
    Downloads DOP images from the 1960s for a single coordinate point.
    Creates a 20x20m bounding box around the point.
    """
    if output_dir is None:
        output_dir = os.path.join(script_dir, "Downloads/LGL/Historical/1960_1969")
    ...
    wms_url = "https://owsproxy.lgl-bw.de/owsproxy/ows/WMS_LGL-BW_HIST_DOP_1960-1969?"
    ...
    try:
        ...
        if not found_usable_image:
            print("\n❌ No usable image found for given coordinates in any year of the 60s")
            return False
        return True
...
def get_LGL_1960s_DOP_by_bbox(lat1, lon1, lat2, lon2, output_dir=None):
    """
    Downloads DOP images from the 1960s for a bounding box defined by two coordinates:
    ...
    try:
        ...
        if not found_usable_image:
            print("\n❌ No usable image found for given bbox in any year of the 60s")
            return False
        return True
...
def get_input():
    ...
    try:
        ...
        if len(parts) == 2:
            ...
            lat, lon = parts
            print(f"\nDownloading 1960s DOP for coordinate: {lat}, {lon}")
            return get_LGL_1960s_DOP_by_coord(lat, lon)
            
        elif len(parts) == 4:
            # Bounding box mode
            lat1, lon1, lat2, lon2 = parts
            print(f"\nDownloading 1960s DOP for bounding box: ({lat1}, {lon1}) to ({lat2}, {lon2})")
            return get_LGL_1960s_DOP_by_bbox(lat1, lon1, lat2, lon2)
